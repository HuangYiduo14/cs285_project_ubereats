...
def get_env_kwargs(env_name, schedule_name=None):
    if env_name in ['MsPacman-v0', 'PongNoFrameskip-v4']:
        ...
        kwargs['exploration_schedule'] = lander_exploration_schedule(kwargs['num_timesteps'], schedule_name)
...
def lander_exploration_schedule(num_timesteps, schedule_name = None):
    if schedule_name is None:
        schedule_name = 'Piecewise'
    ...
    if schedule_name == 'Annealing':
        return AnnealingSchedule(1,0.22, 0.02)
...
class AnnealingSchedule(object):
    def __init__(self, x_scale=1, value_scale=1, final_p=0.02):
        self.x_scale = x_scale
        self.value_scale = value_scale
        self.final_p = final_p
    def value(self,t):
        if t*self.x_scale+1 <= np.exp(self.value_scale):
            return 1.
        eps = self.value_scale/np.log(t*self.x_scale+1)
